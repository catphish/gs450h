 ...
 * V9 - add serial control for settings
 * 
...
uint16_t get_torque(uint8_t gear)
{
  ...
  if(gear==DRIVE)   return map(throttle, 0, 1000, regen,  config.max_torque_fwd);
  if(gear==REVERSE) return map(throttle, 0, 1000, regen, -config.max_torque_rev);
